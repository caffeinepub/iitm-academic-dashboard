    ...
    >
      ...
      {courses.length === 0 ? (
        ...
        <>
          {/* Summary bar */}
          {(() => {
            const allPcts = courses.map((c) => {
              const s = calcAttendance(attendance, c.id);
              const mb = getManualBase(c.id);
              const adjA = s.attended + (mb?.attended ?? 0);
              const adjT = s.total + (mb?.total ?? 0);
              return adjT === 0 ? 0 : Math.round((adjA / adjT) * 100);
            });
            const lowCount = allPcts.filter((p) => p < 75).length;
            ...
            return (
              ...
              >
                {lowCount > 0 && (
                  ...
                  >
                    ⚠️ {lowCount} subject{lowCount > 1 ? "s" : ""} below 75%
                  </div>
                )}
                <div
                  style={{
          ...
          >
            {courses.map((c) => {
              ...
              const attendanceStatus =
                adjPct < 75 ? "LOW" : adjPct <= 85 ? "SAFE" : "GOOD";
              const sc =
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <div
                      style={{
                        padding: "3px 8px",
                        borderRadius: 20,
                        background: sc.replace("0.9)", "0.1)"),
                        border: `1px solid ${sc.replace("0.9)", "0.25)")}`,
                        color: sc,
                        fontSize: 11,
                        fontWeight: 700,
                        display: "inline-flex",
                  ...
                  <ProgressBar percentage={adjPct} color={sc} />
