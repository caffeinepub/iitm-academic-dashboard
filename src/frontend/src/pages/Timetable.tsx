...
import type { Course, EveningSlot } from "../types";
import { PASTEL_COLORS as _PASTEL } from "../utils/slots";
import {
  PASTEL_COLORS,
  SLOT_GRID,
  SLOT_OCCURRENCES,
...
interface Props {
  ...
  onAddEveningSlot: (s: Omit<EveningSlot, "id">) => void;
  onDeleteEveningSlot: (id: string) => void;
}
...
export function Timetable({
  ...
  const [evName, setEvName] = useState("Happiness of Living");
  const [evVenue, setEvVenue] = useState("");
  const [evStart, setEvStart] = useState("18:00");
  const [evEnd, setEvEnd] = useState("20:00");
  const [evDays, setEvDays] = useState<number[]>([0, 1, 2, 3, 4]);
      ...
      <div style={{ marginTop: 32 }}>
        ...
        >
          <div>
            ...
            <p style={{ fontSize: 12, color: "#606880", margin: "3px 0 0" }}>
              Happiness of Living, NSO, extra sessions (6–8 PM)
            </p>
          </div>
          <motion.button
            type="button"
            whileTap={{ scale: 0.95 }}
            className="btn-gradient"
            style={{ fontSize: 12, padding: "7px 16px", borderRadius: 10 }}
            onClick={() => setShowEveningForm((v) => !v)}
          >
            {showEveningForm ? "Cancel" : "+ Add"}
          </motion.button>
        </div>
              ...
              <GlassCard>
                ...
                >
                  ...
                  >
                    <div>
                      ...
                      <input
                        ...
                        placeholder="Happiness of Living"
                        style={{
                          width: "100%",
                      ...
                      {DAY_SHORT.map((d, i) => (
                        <button
                          ...
                          style={{
                            ...
                            background: evDays.includes(i)
                              ? "linear-gradient(90deg,#8b5cf6,#3b82f6)"
                              : "rgba(255,255,255,0.07)",
                            color: evDays.includes(i) ? "#fff" : "#A9B0C7",
                            fontSize: 12,
                            cursor: "pointer",
                            fontWeight: evDays.includes(i) ? 700 : 400,
                          }}
                        >
                          {d}
                        </button>
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.97 }}
                    className="btn-gradient"
                    style={{ padding: "9px 0", borderRadius: 10, fontSize: 13 }}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                <div
                  style={{
                    ...
                    height: 8,
                    borderRadius: "50%",
                    background: slot.color ?? "#D4B8F0",
                    flexShrink: 0,
                  }}
                />
                <div>
                  <div
                    style={{ fontSize: 14, fontWeight: 700, color: "#E0D5FF" }}
                  >
                    {slot.name}
                  </div>
                  <div style={{ fontSize: 12, color: "#7B82A0", marginTop: 2 }}>
                    ...
                    {" · "}
                    {slot.days.map((d) => DAY_SHORT[d]).join(", ")}
                  </div>
                </div>
              </div>
              <button
                type="button"
                onClick={() => onDeleteEveningSlot(slot.id)}
                style={{
